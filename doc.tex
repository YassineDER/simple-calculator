...
Le calcul se fait avec l'aide de reduce, qui permet d'appliquer une fonction de calcul sur chaque élément du tableau, en prenant en compte le résultat précédent.

\subsection{Conteneurisation}
Pour la conteneurisation, j'ai créé un fichier \textit{Dockerfile} qui contient les instructions pour construire l'image du conteneur NodeJS, comme fait dans le TP précédent.
...
J'ai modifié le fichier du deploiement pour ajouter des replicas aux pods. Lorsque l'un des pods est supprimé, un autre est créé pour le remplacer, ce qui assure la scalabilité de l'application. \\
...
Enfin, on peut voir l'adresse IP du service exposé en utilisant la commande:
\begin{verbatim}
